...
struct VulkanDevice {
  ...
  void initialize_logical_device(VkSurfaceKHR surface) {
    QueueFamilyIndices indices =
        search_queue_families(physical_device, surface);

    std::vector<VkDeviceQueueCreateInfo> device_queue_ci;
    std::set<uint32_t> unique_queue_families = {indices.graphics_qf,
                                                indices.present_qf};

    float queue_priority = 1.0f;
    for (uint32_t queueFamily : unique_queue_families) {
      VkDeviceQueueCreateInfo queue_ci =
          vki::deviceQueueCreateInfo(queueFamily, 1, &queue_priority);
      device_queue_ci.push_back(queue_ci);
    }

    VkPhysicalDeviceFeatures device_features = {};
    device_features.samplerAnisotropy = VK_TRUE;
    VkDeviceCreateInfo logical_device_ci = vki::deviceCreateInfo(
        device_queue_ci.size(), device_queue_ci.data(),
        required_validation_layers.size(), required_validation_layers.data(),
        required_device_extensions.size(), required_device_extensions.data(),
        &device_features);

    if (vkCreateDevice(physical_device, &logical_device_ci, nullptr,
  ...
  bool device_valid(VkPhysicalDevice device, VkSurfaceKHR surface) {
    ...
    if (extensions_supported) {
      ...
      swapchain_supported = !swapchain_support_details.formats.empty() &&
    ...
    std::cerr << "asdf: " << indices.qf_completed() << extensions_supported
              << swapchain_supported << features_supported.samplerAnisotropy
              << '\n';
    return indices.qf_completed() && extensions_supported &&
           swapchain_supported;
  }
  ...
  uint32_t find_memory_type(uint32_t filter, VkMemoryPropertyFlags properties) {
    ...
    throw std::runtime_error(
        "Could not find a compatible memory type for buffer");
  }

  VkFormat find_format(std::vector<VkFormat> formats, VkImageTiling tiling,
                       VkFormatFeatureFlags features) {
    for (uint32_t i = 0; i < formats.size(); i++) {
      VkFormatProperties properties;
      vkGetPhysicalDeviceFormatProperties(physical_device, formats[i],
                                          &properties);
  ...
  void destroy() { vkDestroyDevice(logical_device, nullptr); }
};
...
bool check_validation_layer_support() {
  ...
  for (uint32_t i = 0; i < required_validation_layers.size(); i++) {
    ...
    for (uint32_t j = 0; j < available_layers.size(); j++) {
      if (strcmp(required_validation_layers[i],
