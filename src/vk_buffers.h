...
void create_buffer(VulkanDevice device, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VulkanBuffer &buffer)
{
	VkBufferCreateInfo buffer_ci = vki::bufferCreateInfo(size, usage, VK_SHARING_MODE_EXCLUSIVE);

	VkResult create_buffer_result = vkCreateBuffer(device.logical_device, &buffer_ci, nullptr, &buffer.buffer);
	if(create_buffer_result != VK_SUCCESS)
	{
		throw std::runtime_error("Could not create buffer");
	}
	...
	VkMemoryRequirements memreqs;
	vkGetBufferMemoryRequirements(device.logical_device, buffer.buffer, &memreqs);

	VkMemoryAllocateInfo memory_ai = vki::memoryAllocateInfo(memreqs.size, device.find_memory_type(memreqs.memoryTypeBits, properties));

	VkResult mem_alloc_result = vkAllocateMemory(device.logical_device, &memory_ai, nullptr, &buffer.memory);
	if(mem_alloc_result != VK_SUCCESS)
	{
		throw std::runtime_error("failed to allocate buffer memory");
	}

	vkBindBufferMemory(device.logical_device, buffer.buffer, buffer.memory, 0);
}
...
void copy_buffer(VulkanDevice device, VulkanBuffer src, VulkanBuffer dst, VkCommandPool command_pool, VkDeviceSize size)
{
	VkCommandBufferAllocateInfo cmdbuf_ai = vki::commandBufferAllocateInfo(nullptr, command_pool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1);
	...
	copy.dstOffset	  = 0;
	copy.size		  = size;
	vkCmdCopyBuffer(command_buffer, src.buffer, dst.buffer, 1, &copy); // execute the copy command on this command buffer
...
void copy_buffer_to_image(VulkanDevice device, VkCommandPool command_pool, VulkanBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
	VkCommandBuffer command_buffer = begin_command_buffer(device, command_pool);
	...
	vkCmdCopyBufferToImage(command_buffer, buffer.buffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &image_region);
	end_command_buffer(device, command_pool, command_buffer);
}
