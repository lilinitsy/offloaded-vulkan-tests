...
VkPresentInfoKHR presentInfoKHR(uint32_t waitSemaphoreCount,
...
inline VkViewport viewport(float x, float y, float width, float height,
...
commandBufferAllocateInfo(const void *pNext, VkCommandPool commandPool,
...
// Only declare stype, everything else can probably be explicitly stated in its
// creation
...
inline VkShaderModuleCreateInfo shaderModuleCreateInfo(size_t codeSize,
...
pipelineMultisampleStateCreateInfo(VkBool32 sampleShadingEnable,
...
imageSubresourceRange(VkImageAspectFlags aspectMask, uint32_t baseMipLevel,
...
imageSubresourceLayers(VkImageAspectFlags aspectMask, uint32_t mipLevel,
                       uint32_t baseArrayLevel, uint32_t layerCount) {
...
inline VkSamplerCreateInfo samplerCreateInfo(
    VkFilter magFilter, VkFilter minFilter, VkSamplerMipmapMode mipmapMode,
    VkSamplerAddressMode addressModeU, VkSamplerAddressMode addressModeV,
    VkSamplerAddressMode addressModeW, float mipLodBias, float maxAnisotropy,
...
framebufferCreateInfo(VkRenderPass renderPass, uint32_t attachmentCount,
...
descriptorSetLayoutCreateInfo(uint32_t bindingCount,
...
descriptorPoolCreateInfo(uint32_t maxSets, uint32_t poolSizeCount,
...
writeDescriptorSet(VkDescriptorSet dstSet, uint32_t dstBinding,
  ...
  VkWriteDescriptorSet write_descriptor_set = {
      ...
      .descriptorType = descriptorType,
      .pBufferInfo =
          pBufferInfo, // only needed for descriptors that refer to buffer data
  };
...
writeDescriptorSet(VkDescriptorSet dstSet, uint32_t dstBinding,
  ...
  VkWriteDescriptorSet write_descriptor_set = {
      ...
      .descriptorType = descriptorType,
      .pImageInfo =
          pImageInfo, // only needed for descriptors that refer to buffer data
  };
